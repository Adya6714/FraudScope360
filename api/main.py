...
def load_everything():
    ...
    # 4) Extract & save feature columns
    ...
    # 5e) NLP ← train on real txn‐texts
    logger.info("Preparing NLP training corpus from history…")
    # use “merchant country” as a stand‐in “ticket” for each txn
    texts = (
    ...
    explainer_nlp = shap.LinearExplainer(
        nlp.model,
        nlp.vec.transform(texts[:500])   # sample for speed
    )
